...
const teams = [
  ...
  { name: 'Cheapskates',         logo: '/images/mml-logo.png',                  color: '#0F766E', division: 'Relegated',       owner: 'TBD',  seasons: 4, average: '10.50', championships: 0 },
  ...
  { name: 'Deep Sky',            logo: '/images/teams/deep-sky.png',            color: '#0284C7', division: 'Great Plains',    owner: 'Jeff', seasons: 6, average: '7.67',  championships: 0 },
  ...
  { name: 'HemoGoblins',         logo: '/images/teams/hemo-goblins.png',        color: '#B91C1C', division: 'South Central',   owner: 'Sam',  seasons: 6, average: '5.00',  championships: 3 },
  ...
  { name: 'Mowers',              logo: '/images/teams/mowers.png',              color: '#15803D', division: 'Great Lakes',     owner: 'Jeff', seasons: 6, average: '11.33', championships: 0 },
  ...
  { name: 'Sin',                 logo: '/images/teams/sin.png',                 color: '#0f0f0f', division: 'South Central',   owner: 'Joel', seasons: 6, average: '7.17',  championships: 1 },
  ...
  { name: 'Trinitarians',        logo: '/images/teams/trinitarians.png',        color: '#D97706', division: 'New England',     owner: 'Joel', seasons: 6, average: '7.67',  championships: 0 },
  ...
  { name: 'Vanilla Thunder',     logo: '/images/teams/vanilla-thunder.png',     color: '#CA8A04', division: 'Great Lakes',     owner: 'Joel', seasons: 6, average: '12.67', championships: 0 },
...
const DIVISION_ORDER = [
  'Pacific Coast', 'Rocky Mountains', 'South Central', 'South East',
  'South West', 'Great Plains', 'Great Lakes', 'New England', 'Relegated',
]
...
function TeamBanner({ team }: { team: typeof teams[number] }) {
  ...
  return (
    <Link to={`/teams/${slugify(team.name)}`} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ display: 'flex', alignItems: 'center', position: 'relative', overflow: 'hidden', height: 72, marginBottom: 6, textDecoration: 'none', background: team.color, transform: hovered ? 'translateX(4px)' : 'translateX(0)', boxShadow: hovered ? `0 4px 20px ${team.color}55` : 'none', transition: 'all 0.18s ease' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, paddingLeft: 20, flex: 1 }}>
        <span style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 900, fontSize: 24, textTransform: 'uppercase', color: '#fff', letterSpacing: '0.04em', lineHeight: 1, display: 'block', textShadow: '0 1px 4px rgba(0,0,0,0.4)' }}>{team.name}</span>
        {team.championships > 0 && (
          <span style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)', display: 'block', marginTop: 2 }}>{team.championships}× Champion</span>
        )}
      </div>
      <div className="banner-stats" style={{ position: 'relative', zIndex: 1, display: 'flex', gap: 20, paddingRight: 16, alignItems: 'center' }}>
        {[{ label: 'Owner', val: team.owner }, { label: 'Seasons', val: String(team.seasons) }, { label: 'Avg. Place', val: team.average }].map((item) => (
      ...
      <div className="banner-arrow" style={{ position: 'relative', zIndex: 1, width: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.2)', alignSelf: 'stretch', color: 'rgba(255,255,255,0.6)', fontSize: 16, flexShrink: 0 }}>→</div>
    </Link>
  )
}

function divSlug(division: string) { return division.toLowerCase().replace(/\s+/g, '-') }
...
function TeamsListPage() {
  ...
  const activeDivisions = DIVISION_ORDER.filter((d) => d !== 'Relegated' && teams.some((t) => t.division === d))
    ...
    <main style={{ background: '#fff' }}>
      ...
      <section style={{ background: '#1a2a6e', position: 'relative', overflow: 'hidden', paddingBottom: 0 }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 1000, margin: '0 auto', padding: '3.5rem 1.5rem 0', display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'center', gap: '2rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2, alignSelf: 'center', paddingBottom: '2rem' }}>
            {[{ num: String(teams.length), label: 'Total Franchises' }, { num: String(activeDivisions.length), label: 'Active Divisions' }, { num: '6', label: 'Seasons Played' }].map((s) => (
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 1000, margin: '0 auto', padding: '0 1.5rem' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 0, marginTop: '1rem' }}>
            {activeDivisions.map((division) => (
              <button key={division} onClick={() => { scrollToDiv(division); setActiveDiv(division) }}
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
        {DIVISION_ORDER.map((division) => {
          const divTeams = teams.filter((t) => t.division === division).sort((a, b) => parseFloat(a.average) - parseFloat(b.average))
  ...
  return (
    <main style={{ background: '#fff' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '3px solid #C8102E', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)' }}>
        {[{ num: String(team.seasons), label: 'Seasons Played' }, { num: team.average, label: 'Avg. Placement' }, { num: String(team.championships), label: 'Championships' }].map((s, i) => (
              ...
              {results.map((r, i) => (
                <div key={r.season} style={{ display: 'grid', gridTemplateColumns: '80px 80px 1fr 80px', padding: '10px 16px', borderBottom: i < results.length - 1 ? '1px solid #f3f4f6' : 'none', background: r.place === 1 ? '#fff7ed' : '#fff', borderLeft: r.place === 1 ? `3px solid ${team.color}` : '3px solid transparent', alignItems: 'center' }}>
                  ...
                  <span style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 15, color: '#111827', display: 'flex', alignItems: 'center', gap: 6 }}>
                    {r.tournament}{r.place === 1 && <span style={{ fontSize: 14 }}>🏆</span>}
                  </span>
