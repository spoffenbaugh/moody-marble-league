...
const TEAM_THEMES: Record<string, {
  ...
  MML:         { bg: '#1a2a6e', accent: '#C8102E', accentLight: '#e84060', stripe1: '#141f55', stripe2: '#0d1540', textMuted: 'rgba(255,255,255,0.55)' },
}
...
const articles: Article[] = [
  {
    ...
    content: [
      ...
      'The name changed to Yellow Fever sometime before the New Age era. The results, mostly, did not. Across six seasons, Yellow Fever have the worst average placement of any team to compete in all six tournaments: 15.83. An aggregate of 95.',
  ...
  },
  {
    ...
    content: [
      ...
      'Six seasons. An average placement of 16.00. An aggregate score of 96 across all tournaments.',
  ...
  },
  {
    ...
    content: [
      'The numbers are almost cruel. Trinitarians have competed in all six MML seasons. Their average placement is 7.50, among the best in league history.',
      'Season 1: 8th. Season 2: 9th. Season 3: 3rd. Season 4: 16th. Season 5: 3rd again. Season 6: 5th in the finals, watching Sorbetbes hoist the trophy.',
      'What makes it stranger is how they lose. Not in collapses. Not in shocking eliminations. They simply finish near the top in competitions where they were always competitive and never quite first.',
  ...
  },
  {
    ...
    content: [
      ...
      'HemoGoblins established dominance first. Angels were the perennial challengers. Deep Sky sat in the shadows, placing consistently until a 5th-place finish in Season 4 signaled they were ready to move up.',
...
// ─── HERO SLIDESHOW ────────────────────────────────────────────────────────────
const HERO_SLIDES = [
  { teamName: 'MML',         kicker: 'Est. 2016 · Six Seasons',           headline: 'The Moody Marble League',   sub: 'Six seasons. Thirty-six teams. Eight divisions. One championship at a time — this is the full story of the MML.', cta: 'View League History', slug: '/history' },
  { teamName: 'HemoGoblins', kicker: 'Dynasty · Seasons 1–4',             headline: 'The HemoGoblins Legacy',    sub: 'Three titles, one shadow, a dynasty that still defines every future MML contender.',                           cta: 'Read the Story',     slug: '/articles/hemogoblins-legacy' },
  { teamName: 'Sorbetbes',   kicker: 'Season 6 Champions · Iowa Open 2025',headline: 'From Broncos to Sorbetbes', sub: 'Nine years, a name change, and finally the championship that rewrote their history.',                          cta: 'Read the Story',     slug: '/articles/broncos-to-sorbetbes' },
  { teamName: 'Trinitarians',kicker: 'Six Seasons · Zero Championships',   headline: 'The Trinitarians Curse',    sub: 'Second-best average placement in league history. The most tortured fanbase in the sport.',                     cta: 'Read the Story',     slug: '/articles/trinitarians-curse' },
]
...
function HeroSlideshow() {
  ...
  useEffect(() => {
    timerRef.current = setInterval(() => { setCur((c) => (c + 1) % HERO_SLIDES.length) }, 8000)
  ...
  return (
    <section className="relative overflow-hidden select-none" style={{ background: theme.bg, minHeight: 480, transition: 'background 0.6s ease', position: 'relative' }}>
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <img src={slide.teamName === 'MML' ? '/images/mml-logo.png' : getTeamLogo(slide.teamName)} alt="" aria-hidden="true" style={{ position: 'absolute', right: '-2%', top: '50%', transform: 'translateY(-50%)', height: '100%', width: 'auto', objectFit: 'contain', opacity: animating ? 0 : 0.07, filter: 'grayscale(100%) brightness(2)', pointerEvents: 'none', transition: 'opacity 0.4s ease' }} />
      </div>
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: 1280, margin: '0 auto', padding: '3rem 1.5rem 4rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1.5rem', minHeight: 480 }}>
        <div style={{ opacity: animating ? 0 : 1, transform: animating ? 'translateY(16px)' : 'translateY(0)', transition: 'opacity 0.3s ease, transform 0.3s ease', flex: 1, minWidth: 0 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 16, background: 'rgba(255,255,255,0.06)', border: `1px solid ${theme.accent}44`, padding: '5px 14px 5px 10px' }}>
            ...
            <span style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: '0.22em', textTransform: 'uppercase', color: theme.accent }}>{slide.kicker}</span>
          </div>
          <h1 style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 900, fontSize: 'clamp(32px, 7vw, 76px)', lineHeight: 0.92, color: '#ffffff', textTransform: 'uppercase', letterSpacing: '0.02em', marginBottom: '1rem', textShadow: '0 2px 20px rgba(0,0,0,0.4)' }}>{slide.headline}</h1>
          <div style={{ width: 48, height: 3, background: theme.accent, marginBottom: '1rem' }} />
          <p style={{ fontFamily: "'Barlow', Arial, sans-serif", fontWeight: 400, fontSize: 'clamp(14px, 2.5vw, 17px)', lineHeight: 1.6, color: theme.textMuted, marginBottom: '1.5rem', maxWidth: 420 }}>{slide.sub}</p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            <Link to={slide.slug} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: theme.accent, color: '#fff', fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 800, fontSize: 13, letterSpacing: '0.22em', textTransform: 'uppercase', padding: '11px 24px', textDecoration: 'none', transition: 'opacity 0.15s', flexShrink: 0 }}
              onMouseEnter={e => { e.currentTarget.style.opacity = '0.88' }}
              onMouseLeave={e => { e.currentTarget.style.opacity = '1' }}
            >
              {slide.cta}<span style={{ fontSize: 16, lineHeight: 1 }}>→</span>
            </Link>
        ...
        <div style={{ opacity: animating ? 0 : 1, transition: 'opacity 0.35s ease', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          ...
          <img src={slide.teamName === 'MML' ? '/images/mml-logo.png' : getTeamLogo(slide.teamName)} alt={slide.teamName} style={{ width: 'clamp(100px, 18vw, 240px)', height: 'clamp(100px, 18vw, 240px)', objectFit: 'contain', filter: 'drop-shadow(0 8px 32px rgba(0,0,0,0.5))', position: 'relative', zIndex: 1 }} />
        </div>
      </div>

      <div style={{ position: 'absolute', bottom: 16, left: '50%', transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 8, zIndex: 20 }}>
        {HERO_SLIDES.map((s, i) => (
          <button key={s.teamName} onClick={() => { goTo(i); resetTimer() }} aria-label={`Go to slide ${i + 1}`}
    ...
    <div style={{ background: '#fff', borderBottom: '3px solid #C8102E', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)' }}>
      {stats.map((s, i) => (
        <div key={s.label} style={{ textAlign: 'center', padding: '0.75rem 0.5rem', borderRight: i < stats.length - 1 ? '1px solid #e5e7eb' : 'none' }}>
          <span style={{ display: 'block', fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 900, fontSize: 'clamp(28px, 6vw, 40px)', color: '#1a2a6e', lineHeight: 1 }}>{s.num}</span>
          <span style={{ display: 'block', fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 'clamp(9px, 2vw, 11px)', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#6b7280', marginTop: 3 }}>{s.label}</span>
        </div>
      ))}
    </div>
  )
      ...
      <header style={{ background: '#fff', borderBottom: '1px solid #e5e7eb', position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 1.5rem', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }} onClick={() => setMenuOpen(false)}>
            ...
            <div>
              ...
              <div style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 600, fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#9ca3af', marginTop: 1 }}>MML Historical Archive</div>
            </div>
          </Link>

          <nav className="desktop-nav" style={{ alignItems: 'center', gap: 0 }}>
            {[{ label: 'Home', to: '/' }, { label: 'Teams', to: '/teams' }, { label: 'Articles', to: '/articles' }].map((item) => (
            ...
            <div ref={historyRef} style={{ position: 'relative' }} onMouseEnter={() => setHistoryOpen(true)} onMouseLeave={() => setHistoryOpen(false)}>
                ...
                <div style={{ position: 'absolute', top: '100%', left: '50%', transform: 'translateX(-50%)', background: '#fff', border: '1px solid #e5e7eb', borderTop: '3px solid #C8102E', boxShadow: '0 8px 24px rgba(0,0,0,0.10)', minWidth: 240, zIndex: 100 }}>
                  {seasons.map((s) => (
                    <Link key={s.to} to={s.to}
                      ...
                      onMouseEnter={e => { e.currentTarget.style.background = '#f4f6fb'; const l = e.currentTarget.querySelector('.dd-label') as HTMLElement; if (l) l.style.color = '#C8102E' }}
                      onMouseLeave={e => { e.currentTarget.style.background = '#fff'; const l = e.currentTarget.querySelector('.dd-label') as HTMLElement; if (l) l.style.color = '#1a2a6e' }}
                    >
                      <span className="dd-label" style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 13, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#1a2a6e', transition: 'color 0.12s' }}>{s.label}</span>
                      <span style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 11, color: '#9ca3af', letterSpacing: '0.1em' }}>{s.year}</span>
                    </Link>
                  ))}
                </div>
              )}
          ...
          <button className="hamburger-btn" onClick={() => setMenuOpen(!menuOpen)}
            style={{ flexDirection: 'column', gap: 5, background: 'none', border: 'none', cursor: 'pointer', padding: 8 }} aria-label="Toggle menu">
        ...
        {menuOpen && (
          <div style={{ background: '#fff', borderTop: '1px solid #e5e7eb', borderBottom: '3px solid #C8102E' }}>
            ...
            <div style={{ padding: '10px 1.5rem 4px', fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#9ca3af' }}>Seasons</div>
            {seasons.map((s) => (
  ...
  return (
    <footer style={{ background: '#1a2a6e', borderTop: '3px solid #C8102E', marginTop: 0 }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '2rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        <div style={{ fontFamily: "'Barlow', Arial, sans-serif", fontSize: 13, color: 'rgba(255,255,255,0.45)' }}>Competitive marble racing since Season 1 · 2016–2025</div>
        <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
          {[{ label: 'Teams', to: '/teams' }, { label: 'Articles', to: '/articles' }, { label: 'History', to: '/history' }, { label: 'Home', to: '/' }].map((item) => (
            <Link key={item.label} to={item.to}
    ...
    <article onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      <div style={{ padding: '1.25rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1rem' }}>
          <span style={{ background: '#f4f6fb', border: '1px solid #e5e7eb', padding: '3px 10px', fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#1a2a6e' }}>{article.category}</span>
          <img src={getTeamLogo(article.heroTeam)} alt={article.heroTeam} style={{ width: 36, height: 36, objectFit: 'contain', flexShrink: 0 }} />
        </div>
        <h3 style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 800, fontSize: 22, lineHeight: 1.15, textTransform: 'uppercase', color: hovered ? theme.accent : '#111827', letterSpacing: '0.02em', marginBottom: 6, transition: 'color 0.2s' }}>{article.title}</h3>
        <div style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 12, letterSpacing: '0.15em', textTransform: 'uppercase', color: theme.accent, marginBottom: 10 }}>{article.tag}</div>
        <p style={{ fontFamily: "'Barlow', Arial, sans-serif", fontSize: 14, lineHeight: 1.65, color: '#6b7280', flex: 1, marginBottom: '1rem' }}>{article.summary}</p>
        <Link to={`/articles/${article.slug}`} style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 800, fontSize: 12, letterSpacing: '0.2em', textTransform: 'uppercase', color: theme.accent, textDecoration: 'none' }}>Read Article →</Link>
      </div>
    </article>
  )
}
    ...
    <main style={{ background: '#fff' }}>
      <section style={{ background: '#1a2a6e', padding: '3.5rem 1.5rem' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'inline-block', background: '#C8102E', color: '#fff', fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: '0.25em', textTransform: 'uppercase', padding: '4px 12px', marginBottom: 16 }}>MML Journalism</div>
          <h1 style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 900, fontSize: 'clamp(36px, 7vw, 80px)', textTransform: 'uppercase', color: '#fff', letterSpacing: '0.02em', lineHeight: 0.95, marginBottom: '1rem' }}>Featured Articles</h1>
          <p style={{ fontFamily: "'Barlow', Arial, sans-serif", fontSize: 'clamp(14px, 3vw, 18px)', color: 'rgba(255,255,255,0.6)', maxWidth: 560 }}>Deep dives, rivalries, franchise histories, and the stories that shaped the Moody Marble League.</p>
        </div>
      </section>
  ...
  if (!article) {
    return (
      <main style={{ maxWidth: 1280, margin: '0 auto', padding: '4rem 1.5rem' }}>
        <h1 style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 900, fontSize: 48, textTransform: 'uppercase', color: '#1a2a6e' }}>Article Not Found</h1>
        <Link to="/articles" style={{ display: 'inline-block', marginTop: 24, fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 13, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#C8102E', textDecoration: 'none' }}>← Back to Articles</Link>
      </main>
    )
  }
    ...
    <main style={{ background: '#fff' }}>
      <section style={{ background: theme.bg, padding: '3.5rem 1.5rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <Link to="/articles" style={{ display: 'inline-block', fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 12, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', textDecoration: 'none', marginBottom: 20 }}>← Back to Articles</Link>
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
            {relatedTeams.map((name) => <img key={name} src={getTeamLogo(name)} alt={name} style={{ width: 52, height: 52, objectFit: 'contain', background: 'rgba(255,255,255,0.08)', padding: 6 }} />)}
          </div>
          <div style={{ display: 'inline-block', background: theme.accent, color: '#fff', fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: '0.25em', textTransform: 'uppercase', padding: '4px 12px', marginBottom: 14 }}>{article.category}</div>
          <h1 style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 900, fontSize: 'clamp(32px, 6vw, 72px)', textTransform: 'uppercase', color: '#fff', letterSpacing: '0.02em', lineHeight: 0.95, marginBottom: 10 }}>{article.title}</h1>
          <p style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 'clamp(14px, 3vw, 20px)', color: theme.textMuted, letterSpacing: '0.05em' }}>{article.tag}</p>
        </div>
      </section>
      <article style={{ maxWidth: 760, margin: '0 auto', padding: '2.5rem 1.5rem 4rem' }}>
        <p style={{ fontFamily: "'Barlow', Arial, sans-serif", fontSize: 'clamp(16px, 3vw, 20px)', lineHeight: 1.7, color: '#374151', fontWeight: 600, borderLeft: `6px solid ${theme.accent}`, paddingLeft: 18, marginBottom: '2rem' }}>{article.summary}</p>
        <blockquote style={{ background: theme.bg, padding: '1.5rem', marginBottom: '2rem', borderLeft: `6px solid ${theme.accent}` }}>
          <div style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: '0.3em', textTransform: 'uppercase', color: theme.accent, marginBottom: 8 }}>Pull Quote</div>
          <p style={{ fontFamily: "'Barlow Condensed', Arial Narrow, Arial, sans-serif", fontWeight: 800, fontSize: 'clamp(18px, 4vw, 26px)', color: '#fff', lineHeight: 1.2, letterSpacing: '0.01em' }}>"{article.pullQuote}"</p>
        </blockquote>
        {article.content.map((para, i) => (
          <p key={i} style={{ fontFamily: "'Barlow', Arial, sans-serif", fontSize: 'clamp(15px, 3vw, 18px)', lineHeight: 1.8, color: '#374151', marginBottom: '1.25rem' }}>{para}</p>
        ))}
      </article>
    </main>
  )
